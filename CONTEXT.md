...
**Settings-registered models** — the ids a runner's *own* settings file declares usable, read by discovery from the descriptor its manifest carries (`PluginModelDiscovery.settingsModels`: file, dotted path to the rows, which row fields hold id and label) and merged into that runner's catalog. Claude Code is the case it exists for: pointed at an LLM gateway, the models it can run belong to the gateway, and no catalog endpoint can name them — only its `modelPicker` rows can, and those exact strings (`gproxy/default`, not the gateway's bare `default`) are what `--model` accepts. The merge is additive: an id the catalog already has keeps the catalog's entry, and this source can never remove a model. It is also the one discovery source that reads a *user* file.
*Avoid:* "configured models" — it is not about what the user set in Ordewell's own settings (`~/.ordewell/settings.json`), but about a row in the runner's file; and do not confuse it with `fallbackModels`, the static last-resort list.
